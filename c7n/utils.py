...
class IPv4List:
    ...
    def __contains__(self, other):
        if other is None:
            return False
        in_networks = any(
            [other in y_elem for y_elem in self.ipv4_list if isinstance(y_elem, IPv4Network)]
        )
        in_addresses = any(
            [
                other == y_elem
                for y_elem in self.ipv4_list
...
def get_eni_resource_type(eni):
    ...
    # Other Resources
        ...
        return 'nat'
    elif description == 'RDSNetworkInterface' or description.startswith(
        'Network interface for DBProxy'
    ):
        rtype = 'rds'
