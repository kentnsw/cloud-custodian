...
class SubnetFilter(MatchResourceValidator, RelatedResourceFilter):
    ...
    def get_permissions(self):
        ...
        if self.data.get('igw') in (True, False):
            perms += self.manager.get_resource_manager('aws.route-table').get_permissions()
        return perms
    ...
    def match_igw(self, subnet):
        rtable = self.route_tables.get(subnet['SubnetId'], self.route_tables.get(subnet['VpcId']))
        if rtable is None:
    ...
    def process(self, resources, event=None):
        ...
        for r in resources:
            ...
            resource_subnets = self.filter_ignored(
                [
                    related_subnet[sid]
                    for sid in self.subnet.get_related_ids([r])
    ...
    def process_resource(self, r, resource_sgs, resource_subnets, key):
        ...
        if 'security-group' in self.compare:
            ...
            if not self.missing_ok and None in sg_values.values():
                evaluation.append(
                    {'reason': 'SecurityGroupLocationAbsent', 'security-groups': sg_values}
                )
            ...
            if len(sg_space) > self.max_cardinality:
                evaluation.append(
                    {'reason': 'SecurityGroupLocationCardinality', 'security-groups': sg_values}
                )

        if (
            'subnet' in self.compare
            and 'security-group' in self.compare
            and sg_space != subnet_space
        ):
            evaluation.append(
                {
