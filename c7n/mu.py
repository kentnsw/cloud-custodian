...
class LambdaManager:
    ...
    def _create_or_update(self, func, role=None, s3_uri=None, qualifier=None):
        ...
        if existing:
            ...
            if self._update_architecture(
                func, existing, new_config.pop('Architectures', ["x86_64"]), code_ref
            ):
                changed = True
    ...
    def _update_architecture(self, func, existing, new_architecture, code_ref):
        ...
        if diff:
            ...
            params = dict(FunctionName=func.name, Publish=True, Architectures=new_architecture)
            params.update(code_ref)
