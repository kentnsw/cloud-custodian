...
def _default_options(p, exclude=[]):
    ...
    config.add_argument("configs", nargs='*', help="Policy configuration file(s) or directory")
    config.add_argument(
        "-p",
        "--policies",
        default=[],
    ...
    if 'quiet' not in exclude:
        output.add_argument(
            "-q", "--quiet", action="count", help="Less logging (repeatable, -qqq for no output)"
        )
    else:
    ...
    if 'cache' not in exclude:
        p.add_argument(
            "-f",
            "--cache",
            default="~/.cache/cloud-custodian.cache",
...
def _report_options(p):
    ...
    p.add_argument(
        '--no-default-fields', action="store_true", help='Exclude default fields for report.'
    )
    p.add_argument(
        '--format',
        default='csv',
...
def _schema_options(p):
    ...
    p.add_argument('--json', action="store_true", help="Export custodian's jsonschema")
    p.add_argument(
        '--outline',
        action="store_true",
...
def setup_parser():
    ...
    metrics_help = (
        "Emit metrics to provider metrics. Specify 'aws', 'gcp', or 'azure'. "
    ...
    schema_desc = (
        "Browse the available vocabularies (resources, filters, modes, and "
    ...
    schema = subs.add_parser(
        'schema', description=schema_desc, help="Interactive cli docs for policy authors"
    )
    schema.set_defaults(command="c7n.commands.schema_cmd")
    _schema_options(schema)

    report_desc = (
        "Report of resources that a policy matched/ran on. "
        "The default output format is csv, but other formats "
        "are available."
    )
    report = subs.add_parser(
        "report", description=report_desc, help="Tabular report on policy matched resources"
    )
    ...
    version = subs.add_parser('version', help="Display installed version of custodian")
    ...
    deprecations.add_argument(
        "--no-deps",
        dest="check_deprecations",
        action='store_const',
        const=deprecated.SKIP,
...
def main():
    ...
    try:
        ...
        if not callable(command):
            command = getattr(
                importlib.import_module(command.rsplit('.', 1)[0]), command.rsplit('.', 1)[-1]
            )
