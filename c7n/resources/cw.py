...
class SetRuleState(BaseAction):
    ...
    schema = type_schema('set-rule-state', **{'enabled': {'default': True, 'type': 'boolean'}})
    permissions = (
        'events:EnableRule',
        'events:DisableRule',
    )
    ...
    def process(self, resources):
        ...
        for resource in resources:
            try:
                if enabled:
                    retry(client.enable_rule, Name=resource['Name'])
                else:
                    retry(client.disable_rule, Name=resource['Name'])
            except (
