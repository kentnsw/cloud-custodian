...
from c7n.filters import DefaultVpcBase, Filter, ValueFilter
        ...
        for r in resources:
            ...
            if enabled:
                ...
                for fl in flogs:
                    ...
                    fl_match = (
                        status_match
                        and traffic_type_match
                        and dest_match
                        and log_format_match
                        and log_group_match
                        and dest_type_match
                        and delivery_status_match
                    )
...
class VpcSecurityGroupFilter(RelatedResourceFilter):
    ...
    def get_related_ids(self, resources):
        ...
        vpc_group_ids = {
            g['GroupId']
            for g in self.manager.get_resource_manager('security-group').resources()
...
class AttributesFilter(Filter):
    ...
    schema = type_schema(
        'vpc-attributes', dnshostnames={'type': 'boolean'}, dnssupport={'type': 'boolean'}
    )
        ...
        for r in resources:
            ...
            if dns_support is not None:
                support = client.describe_vpc_attribute(
                    VpcId=r['VpcId'], Attribute='enableDnsSupport'
...
class DhcpOptionsFilter(Filter):
    ...
    schema = type_schema(
        'dhcp-options',
        **{
            k: {'oneOf': [{'type': 'array', 'items': {'type': 'string'}}, {'type': 'string'}]}
...
class VpcPostFinding(PostFinding):
    ...
    def format_resource(self, r):
        ...
        detail = {'DhcpOptionsId': r.get('DhcpOptionsId'), 'State': r['State']}

        for assoc in r.get('CidrBlockAssociationSet', ()):
            detail.setdefault('CidrBlockAssociationSet', []).append(
                dict(
        ...
        for assoc in r.get('Ipv6CidrBlockAssociationSet', ()):
            detail.setdefault('Ipv6CidrBlockAssociationSet', []).append(
                dict(
                    AssociationId=assoc['AssociationId'],
...
class Subnet(query.QueryResourceManager):
    ...
    source_mapping = {'describe': DescribeSubnets, 'config': query.ConfigSource}
...
class SecurityGroup(query.QueryResourceManager):
    ...
    source_mapping = {'config': ConfigSG, 'describe': query.DescribeSource}
...
class SecurityGroupDiff:
    ...
    def diff(self, source, target):
        ...
        egress_delta = self.get_rule_delta('IpPermissionsEgress', source, target)
        if egress_delta:
    ...
    def get_tag_delta(self, source, target):
        ...
        return {
            ...
            for k, v in {
                ...
                'updated': {k: target_tags[k] for k in changed},
            }.items()
            if v
        }

    def get_rule_delta(self, key, source, target):
        source_rules = {self.compute_rule_hash(r): r for r in source.get(key, ())}
        target_rules = {self.compute_rule_hash(r): r for r in target.get(key, ())}
...
class SecurityGroupPatch:
    ...
    def apply_delta(self, client, target, change_set):
        ...
        if 'ingress' in change_set:
            self.process_rules(client, 'ingress', target, change_set['ingress'])
    ...
    def process_tags(self, client, group, tag_delta):
        if 'removed' in tag_delta:
            self.retry(
                ...
                Tags=[{'Key': k} for k in tag_delta['removed']],
            )
        tags = []
    ...
    def process_rules(self, client, rule_type, group, delta):
        ...
        revoke, authorize = getattr(client, revoke_op), getattr(client, auth_op)

        # Process removes
        if 'removed' in delta:
            self.retry(
                revoke, GroupId=group['GroupId'], IpPermissions=[r for r in delta['removed']]
            )

        # Process adds
        if 'added' in delta:
            self.retry(
                authorize, GroupId=group['GroupId'], IpPermissions=[r for r in delta['added']]
            )
...
class SGUsage(Filter):
    ...
    def scan_groups(self):
        ...
        for kind, scanner in self.get_scanners():
            ...
            self.log.debug(
                "%s using %d sgs, new refs %s total %s", kind, len(sg_ids), len(new_refs), len(used)
            )
...
class UnusedSecurityGroup(SGUsage):
    ...
    def process(self, resources, event=None):
        ...
        unused = [r for r in resources if r['GroupId'] not in used and 'VpcId' in r]
...
class Stale(Filter):
    ...
    def process(self, resources, event=None):
        ...
        for vpc_id in vpc_ids:
            stale_groups = client.describe_stale_security_groups(VpcId=vpc_id).get(
            ...
            for s in stale_groups:
                if s['GroupId'] in group_map:
                    ...
                    if 'StaleIpPermissionsEgress' in s:
                        r['MatchedIpPermissionsEgress'] = s['StaleIpPermissionsEgress']
                    results.append(r)
...
class SGPermission(Filter):
    ...
    def validate(self):
        ...
        if delta:
            raise PolicyValidationError(
                "Unknown keys %s on %s" % (", ".join(delta), self.manager.data)
            )
        return self
    ...
    def process_self_reference(self, perm, sg_id):
        ...
        if 'UserIdGroupPairs' in perm and 'SelfReference' in self.data:
            self_reference = sg_id in [p['GroupId'] for p in perm['UserIdGroupPairs']]
    ...
    def expand_permissions(self, permissions):
        ...
        for p in permissions:
            np = dict(p)
            values = {}
            for k in (u'IpRanges', u'Ipv6Ranges', u'PrefixListIds', u'UserIdGroupPairs'):
                values[k] = np.pop(k, ())
                np[k] = []
...
class SetPermissions(BaseAction):
    ...
    def validate(self):
        ...
        for perm_type, shape in (('egress', self.egress_shape), ('ingress', self.ingress_shape)):
        ...
        remover = RemovePermissions(
            {
                ...
                'egress': self.data.get('remove-egress', ()),
            },
            self.manager,
        )
...
class NetworkInterface(query.QueryResourceManager):
    ...
    source_mapping = {'describe': DescribeENI, 'config': query.ConfigSource}
...
class DeleteNetworkInterface(BaseAction):
    ...
    def process(self, resources):
        ...
        for r in resources:
            try:
                self.manager.retry(
                    client.delete_network_interface, NetworkInterfaceId=r['NetworkInterfaceId']
                )
...
class MissingRoute(Filter):
    ...
    def process(self, resources, event=None):
        tables = self.manager.get_resource_manager('route-table').resources()
        routed_vpcs = {}
...
class AclAwsS3Cidrs(Filter):
    ...
    def process(self, resources, event=None):
        ...
        cidrs = jmespath.search("PrefixLists[].Cidrs[]", ec2.describe_prefix_lists())
            ...
            for entry in r['Entries']:
                ...
                for c in matched:
                    if c in entry_cidr and matched[c] is None:
                        matched[c] = entry['RuleAction'] == 'allow' and True or False
...
class NetworkAddress(query.QueryResourceManager):
    ...
    source_mapping = {'describe': DescribeElasticIp, 'config': query.ConfigSource}
...
class VPCEndpointFilter(RelatedResourceByIdFilter):
    ...
    schema = type_schema('vpc-endpoint', rinherit=ValueFilter.schema)
...
class UnusedKeyPairs(Filter):
    ...
    schema = type_schema('unused', state={'type': 'boolean'})
...
class CreateFlowLogs(BaseAction):
    ...
    RESOURCE_ALIAS = {'vpc': 'VPC', 'subnet': 'Subnet', 'eni': 'NetworkInterface'}
    ...
    def validate(self):
        ...
        if not self.state:
            return
        destination_type = self.data.get('LogDestinationType', 'cloud-watch-logs')
        ...
        for r in dvalidation.get('absent', ()):
            if r in self.data:
                raise PolicyValidationError(
                    '%s is prohibited for destination-type:%s' % (r, destination_type)
                )
        if (
            'one-of' in dvalidation
            and sum([1 for k in dvalidation['one-of'] if k in self.data]) != 1
        ):
            raise PolicyValidationError(
    ...
    def process(self, resources):
        ...
        try:
            ...
            for r in results['Unsuccessful']:
                self.log.exception(
                    'Exception: create flow-log for %s: %s', r['ResourceId'], r['Error']['Message']
                )
        except ClientError as e:
...
class PrefixListDescribe(query.DescribeSource):
    def get_resources(self, ids, cache=True):
        query = {'Filters': [{'Name': 'prefix-list-id', 'Values': ids}]}
...
class Entry(Filter):
    ...
    schema = type_schema('entry', rinherit=ValueFilter.schema)
...
class SubnetModifyAtrributes(BaseAction):
    ...
    schema = type_schema(
        ...
        PrivateDnsHostnameTypeOnLaunch={'type': 'string', 'enum': ['ip-name', 'resource-name']},
    )
...
class CrossAZRouteTable(Filter):
    ...
    def resolve_subnets(self, resource, subnets):
        return {
            s['SubnetId'] for s in subnets if s[self.table_annotation] == resource['RouteTableId']
        }
        ...
        for t in tables:
            for association in t['Associations']:
                if association.get('SubnetId'):
                    subnets[association['SubnetId']][self.table_annotation] = t['RouteTableId']
    ...
    def process_route_table(self, subnets, nat_subnets, resource):
        ...
        for route in resource['Routes']:
            ...
            mismatch_subnets = {
                s: subnets[s]['AvailabilityZone']
                for s in associated_subnets
    ...
    def process(self, resources, event=None):
        subnets = {
            s['SubnetId']: s for s in self.manager.get_resource_manager('aws.subnet').resources()
        }
        nat_subnets = {
