...
ACCOUNTS_GCP = {
    'projects': [{'project_id': 'custodian-1291', 'name': 'devy'}],
}
...
POLICIES_AWS_DEFAULT = yaml.safe_dump(
    {
        'policies': [
            {'name': 'compute', 'resource': 'aws.ec2', 'tags': ['red', 'green']},
            {'name': 'serverless', 'resource': 'aws.lambda', 'tags': ['red', 'black']},
        ],
    },
    default_flow_style=False,
)
...
class OrgTest(TestUtils):
    ...
    def test_validate_azure_provider(self):
        run_dir = self.setup_run_dir(
            ...
            policies={'policies': [{'name': 'vms', 'resource': 'azure.vm'}]},
        )
    ...
    def test_validate_gcp_provider(self):
        run_dir = self.setup_run_dir(
            ...
            policies={'policies': [{'resource': 'gcp.instance', 'name': 'instances'}]},
        )
        logger = mock.MagicMock()
        run_account = mock.MagicMock()
        run_account.return_value = ({}, True)
    ...
    def test_cli_run_aws(self):
        ...
        logger = mock.MagicMock()
        run_account = mock.MagicMock()
        run_account.return_value = ({'compute': 24, 'serverless': 12}, True)
        self.patch(org, 'logging', logger)
        self.patch(org, 'run_account', run_account)
        self.change_cwd(run_dir)
    ...
    def test_filter_policies(self):
        d = {
            'policies': [
                ...
                {'name': 'find-serverless', 'resource': 'aws.lambda', 'tags': ['blue', 'red']},
            ]
        }
        ...
        self.assertEqual([n['name'] for n in t2['policies']], ['find-serverless'])

        t3 = copy.deepcopy(d)
        org.filter_policies(t3, [], ['find-ml'], [], [])
        self.assertEqual([n['name'] for n in t3['policies']], ['find-ml'])

        t4 = copy.deepcopy(d)
        org.filter_policies(t4, [], [], 'gcp.instance', [])
        self.assertEqual([n['name'] for n in t4['policies']], ['find-ml'])

    def test_resolve_regions_comma_separated(self):
        self.assertEqual(
            org.resolve_regions(
                ['us-west-2,eu-west-1,us-east-1,us-west-2', 'eu-west-1,us-east-2,us-east-1'], None
            ),
    ...
    def test_resolve_regions(self):
        ...
        self.assertEqual(org.resolve_regions(['us-west-2'], account), ['us-west-2'])
        self.assertEqual(org.resolve_regions([], account), ('us-east-1', 'us-west-2'))

    def test_filter_accounts(self):

        d = {
            'accounts': [
                {'name': 'dev', 'account_id': '123456789012', 'tags': ['blue', 'red']},
                {'name': 'prod', 'account_id': '123456789013', 'tags': ['green', 'red']},
            ]
        }
        ...
        self.assertEqual([a['name'] for a in t5['accounts']], ['dev'])

        t6 = copy.deepcopy(d)
        org.filter_accounts(t6, [], [], ['dev'])
        self.assertEqual([a['name'] for a in t6['accounts']], ['prod'])
