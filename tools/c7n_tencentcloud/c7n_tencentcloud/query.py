    ...
    def resources(self, params=None):
        ...
        if self.resource_manager.resource_type.paging_def:
            res = self.query_helper.paged_filter(
                self.resource_manager.config.region, self.resource_manager.resource_type, params
            )
        else:
...
class QueryResourceManager(ResourceManager, metaclass=QueryMeta):
    ...
    def get_source(self, source_type):
        factory = self.source_mapping.get(
            source_type in ("describe", DESC_SOURCE_NAME) and "describe", sources.get(source_type)
        )
        if factory is None:
