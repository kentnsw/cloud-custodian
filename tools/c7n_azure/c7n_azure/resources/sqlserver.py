...
class VulnerabilityAssessmentFilter(ValueFilter):
    ...
    schema = type_schema(
        'vulnerability-assessment', rinherit=ValueFilter.schema, enabled=dict(type='boolean')
    )
    ...
    def __call__(self, resource):
        recurring_scan_enabled = (
            resource[self.key].get('recurringScans', {}).get('isEnabled', False)
        )
    ...
    def _process_resource_set(self, resources, event=None):
        ...
        for resource in resources:
            if 'auditingSettings' not in resource['properties']:
                ...
                resource['properties']['auditingSettings'] = auditing_settings.serialize(True).get(
                    'properties', {}
                )
            ...
            if StringUtils.equal(
                resource['properties']['auditingSettings'].get('state'), required_status
            ):
