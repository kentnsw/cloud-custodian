...
class AzureFunctionMode(ServerlessExecutionMode):
    ...
    def get_function_app_params(self):
        ...
        function_app_name = FunctionAppUtilities.get_function_name(
            self.policy_name, function_suffix
        )
        ...
        if isinstance(settings, str):
            ...
            for key in properties.keys():
                ...
                if isinstance(value, dict):
                    result[key] = AzureFunctionMode.extract_properties(
                        {'v': value}, 'v', properties[key]
                    )
                else:
                    result[key] = value
...
class AzureEventGridMode(AzureFunctionMode):
    ...
    def _create_event_subscription(self, storage_account, queue_name, session):
        ...
        advance_filter = StringInAdvancedFilter(
            key='Data.OperationName', values=self.subscribed_events
        )
        event_filter = EventSubscriptionFilter(advanced_filters=[advance_filter])

        for subscription_id in self.target_subscription_ids:
            try:
                AzureEventSubscription.create(
                    destination, queue_name, subscription_id, session, event_filter
                )
                self.log.info(
