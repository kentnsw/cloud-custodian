...
class WebAppTest(BaseTest):
    ...
    def test_validate_webapp_schema(self):
        with self.sign_out_patch():
            ...
            p = self.load_policy(
                {'name': 'test-azure-webapp', 'resource': 'azure.webapp'}, validate=True
            )
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    ...
                    {'type': 'configuration', 'key': 'minTlsVersion', 'value': '1.2', 'op': 'ne'},
                ],
            }
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    ...
                    {'type': 'authentication', 'key': 'enabled', 'value': False, 'op': 'eq'},
                ],
            }
        )
        resources = p.run()
