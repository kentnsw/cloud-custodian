...
from c7n_azure.resources.storage import (
    StorageSettingsUtilities,
    StorageFirewallRulesFilter,
...
class StorageTest(BaseTest):
    ...
    def test_storage_schema_validate(self):
        with self.sign_out_patch():
            p = self.load_policy(
                {'name': 'test-storage', 'resource': 'azure.storage'}, validate=True
            )
            self.assertTrue(p)
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    ...
                    {'type': 'firewall-rules', 'include': ['1.2.2.129']},
                ],
            }
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    ...
                    {'type': 'firewall-rules', 'any': ['1.2.2.128/25', '8.8.8.8', '10.10.10.10']},
                ],
            }
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    ...
                    {'type': 'firewall-rules', 'any': ['8.8.8.8', '10.10.10.10']},
                ],
            }
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    ...
                    {'type': 'firewall-rules', 'only': ['1.2.2.128/25', '10.10.10.10']},
                ],
            }
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    ...
                    {'type': 'firewall-rules', 'only': ['1.2.2.128/25', '3.1.1.1', '10.10.10.10']},
                ],
            }
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    ...
                    {'type': 'firewall-rules', 'include': ['3.1.1.1', '3.1.1.2-3.1.1.2']},
                ],
            },
            validate=True,
        )
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    ...
                    {'type': 'firewall-rules', 'equal': ['3.1.1.1-3.1.1.1', '1.2.2.128/25']},
                ],
            },
            validate=True,
        )
    ...
    def test_enable_log_settings(self):
        ...
        self.assertTrue(
            blob_settings['analytics_logging'].delete
            and blob_settings['analytics_logging'].read
            and blob_settings['analytics_logging'].write
        )
        self.assertTrue(
            queue_settings['analytics_logging'].delete
            and queue_settings['analytics_logging'].read
            and queue_settings['analytics_logging'].write
        )
        self.assertTrue(
            table_settings.logging.delete
    ...
    def test_storage_settings_get_table_settings(
        self, mock_get_storage_key, mock_get_table_properties
    ):
        mock_storage_account = {
            "resourceGroup": "mock_resource_group",
            "name": "mock_storage_account",
            ...
            p = self.load_policy(
                {
                    ...
                    'actions': [{'type': 'require-secure-transfer', 'value': True}],
                }
            )
            p.run()
    ...
    def test_query_default_deny(self):
        resource = {
            'properties': {
                'networkAcls': {
                    'defaultAction': 'Deny',
                    'ipRules': [{'value': '10.0.0.0/16'}, {'value': '8.8.8.8'}],
                }
            }
        }
