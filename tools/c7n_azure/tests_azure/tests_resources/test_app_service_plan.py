...
class AppServicePlanTest(BaseTest):
    def setUp(self):
        ...
        self.client = local_session(Session).client('azure.mgmt.web.WebSiteManagementClient')
        self.update_mock_path = (
            'azure.mgmt.web.v{}.operations._app_service_plans_operations.'
            'AppServicePlansOperations.update'.format(
                self.client._get_api_version('app_service_plans').replace('-', '_')
            )
        )
            ...
            p = self.load_policy(
                {
                    ...
                    'filters': [
                        ...
                        {'type': 'onhour', 'default_tz': "pt", 'onhour': 18, 'tag': 'schedule'},
                    ],
                    'actions': [{'type': 'resize-plan', 'size': 'F1'}],
                },
                validate=True,
            )
            self.assertTrue(p)
            ...
            p = self.load_policy(
                {
                    ...
                    'filters': [
                        ...
                        {'type': 'value', 'key': 'sku.name', 'op': 'eq', 'value': 'S1'},
                    ],
                    'actions': [{'type': 'resize-plan', 'size': 'B1', 'count': 2}],
                },
                validate=True,
            )
            resources = p.run()
            ...
            p = self.load_policy(
                {
                    ...
                    'actions': [{'type': 'resize-plan', 'size': 'B1', 'count': 3}],
                },
                validate=True,
            )
            resources = p.run()
            ...
            p = self.load_policy(
                {
                    ...
                    'actions': [
                        {'type': 'resize-plan', 'size': {'type': 'resource', 'key': 'tags.sku'}}
                    ],
                }
            )
            resources = p.run()
            ...
            p = self.load_policy(
                {
                    ...
                    'actions': [{'type': 'resize-plan', 'count': 3}],
                },
                validate=True,
            )
            resources = p.run()
