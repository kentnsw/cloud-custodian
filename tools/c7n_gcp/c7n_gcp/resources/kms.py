...
class KmsKeyRing(QueryResourceManager):
    class resource_type(TypeInfo):
        ...
        def get(client, resource_info):
            name = 'projects/{}/locations/{}/keyRings/{}'.format(
...
class KmsCryptoKey(ChildResourceManager):
    ...
    def _get_parent_resource_info(self, child_instance):
        project_id, location, key_ring_id = re.match(
            'projects/(.*?)/locations/(.*?)/keyRings/(.*?)/cryptoKeys/.*', child_instance['name']
        ).groups()
        return {'project_id': project_id, 'location': location, 'key_ring_id': key_ring_id}
...
class KmsCryptoKeyVersion(ChildResourceManager):
    ...
    def _get_parent_resource_info(self, child_instance):
        ...
        project_id, location, key_ring_id, crypto_key_id = re.match(
            path, child_instance['name']
        ).groups()
        return {
