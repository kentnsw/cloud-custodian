...
class PeriodicMode(FunctionMode, PullMode):
    ...
    def _get_function(self):
        events = [
            mu.PeriodicEvent(
                local_session(self.policy.session_factory), self.policy.data['mode'], self.region
            )
        ]
...
class SecurityCenterMode(FunctionMode):
    ...
    schema = type_schema(
        'gcp-scc', org={'type': 'integer'}, required=['org'], rinherit=FunctionMode.schema
    )
    ...
    def _get_function(self):
        events = [
            mu.PubSubSource(
                local_session(self.policy.session_factory), {"topic": self._resource_topic()}
            ),
            mu.SecurityCenterSubscriber(
                local_session(self.policy.session_factory),
                {"topic": self._resource_topic(), "org": self.policy.data["mode"]["org"]},
