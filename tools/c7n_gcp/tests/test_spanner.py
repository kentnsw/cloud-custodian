...
class SpannerInstanceTest(BaseTest):
    def test_spanner_instance_query(self):
        ...
        policy = {'name': 'all-spanner-instances', 'resource': 'gcp.spanner-instance'}
    ...
    def test_spanner_instance_get(self):
        ...
        self.assertEqual(
            instances[0]['config'], 'projects/cloud-custodian/instanceConfigs/regional-asia-east1'
        )
        self.assertEqual(
            instances[0]['name'], 'projects/cloud-custodian/instances/custodian-spanner-1'
        )
        self.assertEqual(
    ...
    def test_spanner_instance_delete(self):
        ...
        base_policy = {'name': 'spanner-instance-delete', 'resource': 'gcp.spanner-instance'}
        policy = self.load_policy(
            dict(
                base_policy,
        ...
        result = client.execute_query('list', {'parent': 'projects/' + project_id})
    ...
    def test_spanner_instance_patch_node_count(self):
        ...
        base_policy = {'name': 'spanner-instance-patch', 'resource': 'gcp.spanner-instance'}
        policy = self.load_policy(
            dict(
                base_policy,
                filters=[{'type': 'value', 'key': 'nodeCount', 'value': 1, 'op': 'greater-than'}],
                actions=[{'type': 'set', 'nodeCount': 1}],
            ),
            session_factory=session_factory,
        )
    ...
    def test_spanner_instance_set_iam_policy_add(self):
                ...
                'actions': [
                    {
                        'type': 'set-iam-policy',
                        'add-bindings': [
                            {'members': ['user:yauhen_shaliou@comelfo.com'], 'role': 'roles/owner'},
                            {'members': ['user:alex.karpitski@gmail.com'], 'role': 'roles/viewer'},
                            {'members': ['user:mediapills@gmail.com'], 'role': 'roles/editor'},
                        ],
                    }
                ],
            },
        ...
        self.assertEqual(
            ...
            [
                {'members': ['user:yauhen_shaliou@comelfo.com'], 'role': 'roles/owner'},
                {'members': ['user:dkhanas@gmail.com'], 'role': 'roles/viewer'},
            ],
        )
        ...
        self.assertEqual(
            ...
            [
                ...
                {'members': ['user:yauhen_shaliou@comelfo.com'], 'role': 'roles/owner'},
                {
                    'members': ['user:alex.karpitski@gmail.com', 'user:dkhanas@gmail.com'],
                    'role': 'roles/viewer',
                },
            ],
        )
    ...
    def test_spanner_instance_set_iam_policy_remove(self):
        ...
        resource_full_name = 'projects/%s/instances/%s' % (project_id, resource_name)
        session_factory = self.replay_flight_data(
            'spanner-instance-set-iam-policy-remove', project_id=project_id
        )
        policy = self.load_policy(
            {
...
class SpannerDatabaseInstanceTest(BaseTest):
    def test_spanner_database_instance_query(self):
        ...
        policy = self.load_policy(
            {'name': 'all-spanner-database-instances', 'resource': 'gcp.spanner-database-instance'},
            session_factory=session_factory,
        )
                ...
                'actions': [
                    {
                        'type': 'set-iam-policy',
                        'remove-bindings': [
                            {'role': 'roles/owner', 'members': ['user:yauhen_shaliou@comelfo.com']},
