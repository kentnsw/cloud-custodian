...
class FunctionTest(BaseTest):
    def test_delete(self):
        factory = self.replay_flight_data('function-delete', project_id='cloud-custodian')
        p = self.load_policy(
            {
                'name': 'func-del',
                'resource': 'gcp.function',
                'filters': [{'httpsTrigger': 'present'}, {'entryPoint': 'hello_http'}],
                'actions': ['delete'],
            },
        ...
        func = client.execute_query('get', {'name': resources[0]['name']})
        self.maxDiff = None
