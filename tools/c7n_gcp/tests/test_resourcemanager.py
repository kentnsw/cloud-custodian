...
class LimitsTest(BaseTest):
    ...
    def test_policy_resource_limits_count(self):
        ...
        p = self.load_policy(
            {'name': 'limits', 'resource': 'gcp.disk', 'max-resources': 1},
            session_factory=session_factory,
        )
...
class OrganizationTest(BaseTest):
    def test_project_get(self):
        ...
        p = self.load_policy(
            {'name': 'organization', 'resource': 'gcp.organization'}, session_factory=factory
        )
    ...
    def test_organization_query(self):
        ...
        policy = self.load_policy(
            {'name': 'gcp-organization-dryrun', 'resource': 'gcp.organization'},
            session_factory=session_factory,
        )
                ...
                'actions': [
                    {
                        'type': 'set-iam-policy',
                        'add-bindings': [
                            {'members': ['user:mediapills@gmail.com'], 'role': 'roles/owner'}
                        ],
                    }
                ],
            },
...
class FolderTest(BaseTest):
    def test_folder_query(self):
        ...
        policy = self.load_policy(
            {'name': 'gcp-folder-dryrun', 'resource': 'gcp.folder', 'query': [{'parent': parent}]},
            session_factory=session_factory,
        )
...
class ProjectTest(BaseTest):
    ...
    @pytest.mark.skipif(sys.platform.startswith('win'), reason='windows file path fun')
    def test_propagate_tags(self):
        ...
        p = self.load_policy(
            {
                ...
                'query': [{'filter': 'parent.id:389734459213 parent.type:folder'}],
                'filters': [{'tag:cost-center': 'absent'}],
                'actions': [
                    {'type': 'propagate-labels', 'folder-labels': {'url': 'file://%s' % label_path}}
                ],
            },
        ...
        project = client.execute_query('get', {'projectId': 'c7n-test-target'})
    ...
    def test_project_hierarchy_no_op(self):
        ...
        factory = self.replay_flight_data('project-hierarchy')
        p = self.load_policy(
            {'name': 'p-parents', 'resource': 'gcp.project'}, session_factory=factory
        )
    ...
    def test_project_delete(self):
        ...
        client = p.resource_manager.get_client()
        project = client.execute_query('get', {'projectId': 'hautomation'})
    ...
    def test_project_set_iam_policy(self):
        ...
        get_iam_policy_params = {'resource': resource_full_name, 'body': {}}
        session_factory = self.replay_flight_data('project-set-iam-policy')
                ...
                'actions': [
                    {
                        'type': 'set-iam-policy',
                        'add-bindings': [
                            {'members': ['user:mediapills@gmail.com'], 'role': 'roles/automl.admin'}
                        ],
                    }
                ],
            },
        ...
        actual_bindings = client.execute_query('getIamPolicy', get_iam_policy_params)
        expected_bindings = [
            {'members': ['user:alex.karpitski@gmail.com'], 'role': 'roles/automl.admin'},
            {'members': ['user:alex.karpitski@gmail.com'], 'role': 'roles/billing.projectManager'},
            {'members': ['user:alex.karpitski@gmail.com'], 'role': 'roles/owner'},
        ]
        self.assertEqual(actual_bindings['bindings'], expected_bindings)
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    {
                        'type': 'iam-policy',
                        'user-role': {'user': "abcdefg", 'has': True, 'role': 'roles/admin'},
                    }
                ],
            },
            session_factory=factory,
        ...
        p = self.load_policy(
            {
                'name': 'resource',
                'resource': 'gcp.project',
                'filters': [{'type': 'missing', 'policy': {'resource': 'gcp.bucket'}}],
            },
            session_factory=factory,
        )
        ...
        p = self.load_policy(
            {
                'name': 'gcp-access-approval',
                'resource': 'gcp.project',
                "filters": [{'type': 'access-approval', 'key': 'name', 'value': 'absent'}],
            },
            session_factory=session_factory,
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                ...
                "resource": "gcp.organization",
                "filters": [
                    {'type': 'essential-contacts', 'attrs': [{'validationState': 'INVALID'}]}
                ],
            },
            session_factory=session_factory,
        )
