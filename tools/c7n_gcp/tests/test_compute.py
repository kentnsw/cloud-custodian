...
class InstanceTest(BaseTest):
    ...
    def test_instance_get(self):
        ...
        p = self.load_policy(
            {'name': 'one-instance', 'resource': 'gcp.instance'}, session_factory=factory
        )
        ...
        p = self.load_policy(
            {
                ...
                'actions': [{'type': 'mark-for-op', 'op': 'start'}],
            },
            session_factory=factory,
        )
        resources = p.run()
...
class DiskTest(BaseTest):
    ...
    def test_disk_delete(self):
        ...
        zone = resources[0]['zone'].rsplit('/', 1)[-1]
        result = client.execute_query(
            'list', {'project': project_id, 'filter': 'name = instance-1', 'zone': zone}
        )
...
class ImageTest(BaseTest):
    def test_image_query(self):
        factory = self.replay_flight_data('image-query', project_id='cloud-custodian')
        p = self.load_policy(
            {'name': 'all-images', 'resource': 'gcp.image'}, session_factory=factory
        )
        resources = p.run()
        self.assertEqual(len(resources), 1)
...
class InstanceTemplateTest(BaseTest):
    def test_instance_template_query(self):
        ...
        policy = self.load_policy(
            {'name': 'gcp-instance-template-dryrun', 'resource': 'gcp.instance-template'},
        ...
        policy = self.load_policy(
            {
                ...
                'mode': {'type': 'gcp-audit', 'methods': ['beta.compute.instanceTemplates.insert']},
            },
            session_factory=session_factory,
        )
    ...
    def test_instance_template_delete(self):
        ...
        try:
            result = client.execute_query(
                'get', {'project': project_id, 'instanceTemplate': resource_name}
            )
...
class AutoscalerTest(BaseTest):
        ...
        policy = self.load_policy(
            {
                ...
                'mode': {'type': 'gcp-audit', 'methods': ['v1.compute.autoscalers.insert']},
            },
            session_factory=session_factory,
        )
    ...
    def test_autoscaler_set(self):
                ...
                'actions': [
                    {
                        ...
                        'loadBalancingUtilization': {'utilizationTarget': 0.7},
                        'minNumReplicas': 1,
                        'maxNumReplicas': 4,
                    }
                ],
        ...
        result = client.execute_query(
            'list',
            {'project': project_id, 'zone': 'us-central1-a', 'filter': 'name = instance-group-2'},
        )
