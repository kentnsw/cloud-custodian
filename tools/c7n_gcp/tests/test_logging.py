...
class LogProjectSinkTest(BaseTest):
    def test_query(self):
        ...
        p = self.load_policy(
            {'name': 'log-project-sink', 'resource': 'gcp.log-project-sink'},
    ...
    def test_get_project_sink(self):
        ...
        p = self.load_policy(
            {
                'name': 'log-project-sink-resource',
    ...
    def test_delete_project_sink(self):
        ...
        factory = self.replay_flight_data('log-project-sink-delete', project_id)
        policy = self.load_policy(
            {
                'name': 'log-project-sink-delete',
                'resource': 'gcp.log-project-sink',
        ...
        sinkName = 'projects/{project_id}/sinks/{name}'.format(
            project_id=project_id, name=resource_name
        )
...
class LogProjectMetricTest(BaseTest):
    def test_query(self):
        ...
        p = self.load_policy(
            {'name': 'log-project-metric', 'resource': 'gcp.log-project-metric'},
            session_factory=factory,
        )
        resource = p.run()
        self.assertEqual(len(resource), 1)
    ...
    def test_get_project_metric(self):
        ...
        factory = self.replay_flight_data('log-project-metric-query', project_id)
        p = self.load_policy(
            {
...
class LogExclusionTest(BaseTest):
    def test_query(self):
        ...
        p = self.load_policy(
            {'name': 'log-exclusion', 'resource': 'gcp.log-exclusion'}, session_factory=factory
        )
        resource = p.run()
        self.assertEqual(len(resource), 1)
        self.assertEqual(
