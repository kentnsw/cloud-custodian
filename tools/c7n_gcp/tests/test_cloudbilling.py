...
class CloudBillingAccountTest(BaseTest):
    def test_billingaccount_query(self):
        ...
        session_factory = self.replay_flight_data('cloudbilling-account-query')

        policy = self.load_policy(
            {'name': 'billing-cloudbilling-account-dryrun', 'resource': 'gcp.cloudbilling-account'},
    ...
    def test_billingaccount_get(self):
        billingaccount_resource_name = 'billingAccounts/CU570D-1A4CU5-70D1A4'
        session_factory = self.replay_flight_data('cloudbilling-account-get')

        policy = self.load_policy(
            {
                'name': 'billing-cloudbilling-account-audit',
                'resource': 'gcp.cloudbilling-account',
                'mode': {'type': 'gcp-audit', 'methods': ['AssignProjectToBillingAccount']},
            },
            session_factory=session_factory,
        )
