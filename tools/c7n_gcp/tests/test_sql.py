...
class SqlInstanceTest(BaseTest):
    def test_sqlinstance_label_params(self):
        p = self.load_policy({'name': 'sql-labels', 'resource': 'gcp.sql-instance'})
        model = p.resource_manager.resource_type
        assert model.get_label_params(
            {'selfLink': 'https://gcp-sql/projects/abc-123/instances/rds-123'}, {'k': 'v'}
        ) == {
            'project': 'abc-123',
            'instance': 'rds-123',
            'body': {'settings': {'userLabels': {'k': 'v'}}},
        }
    ...
    def test_sqlinstance_query(self):
        ...
        p = self.load_policy(
            {'name': 'all-sqlinstances', 'resource': 'gcp.sql-instance'}, session_factory=factory
        )
        resources = p.run()
    ...
    def test_sqlinstance_get(self):
        ...
        p = self.load_policy(
            {'name': 'one-sqlinstance', 'resource': 'gcp.sql-instance'}, session_factory=factory
        )
        instance = p.resource_manager.get_resource(
            {'project_id': 'cloud-custodian', 'database_id': 'cloud-custodian:brenttest-2'}
        )
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    ...
                    {'type': 'value', 'key': 'state', 'op': 'equal', 'value': 'RUNNABLE'},
                    {
                        'type': 'value',
        ...
        policy = self.load_policy(
            {
                ...
                'mode': {'type': 'gcp-audit', 'methods': ['cloudsql.backupRuns.create']},
            },
            session_factory=session_factory,
        )
        ...
        policy = self.load_policy(
            {
                ...
                'mode': {'type': 'gcp-audit', 'methods': ['cloudsql.sslCerts.create']},
            },
            session_factory=session_factory,
        )
