...
class TestAMI(BaseTest):
        ...
        p = self.load_policy(
            {
                ...
                'actions': [{'type': 'remove-launch-permissions'}],
            },
            session_factory=factory,
        )
        resources = p.run()
    ...
    def test_ami_set_permissions_remove_matched(self):
        ...
        assert perms == [
            {
                'OrganizationalUnitArn': 'arn:aws:organizations:112233445566:ou/o-xyz123abc/ou-a123-xyzab234'
            }
        ]
    ...
    def test_ami_set_deprecation_age(self):
        ...
        p = self.load_policy(
            {
                'name': 'ami-check',
                'resource': 'aws.ami',
                'filters': [{'type': 'value', 'key': 'DeprecationTime', 'value': 'absent'}],
                'actions': [{'type': 'set-deprecation', 'age': 45}],
            },
            session_factory=factory,
        )
        resources = p.run()
        ...
        client = factory().client('ec2')
        dtime = client.describe_images(ImageIds=[resources[0]['ImageId']])['Images'][0][
            'DeprecationTime'
        ]
        ...
        p = self.load_policy(
            {
                ...
                'resource': 'aws.ami',
                'filters': [{'type': 'value', 'key': 'DeprecationTime', 'value': 'absent'}],
                'actions': [{'type': 'set-deprecation', 'date': '2020-09-24T13:31'}],
            },
            session_factory=factory,
        )
        resources = p.run()
            ...
            self.load_policy(
                {
                    ...
                    'resource': 'aws.ami',
                    'filters': [{'type': 'value', 'key': 'DeprecationTime', 'value': 'absent'}],
                    'actions': [{'type': 'set-deprecation', 'date': "notright"}],
                }
            )
        self.assertIn("has invalid date format", str(e.exception))
            ...
            self.load_policy(
                {
                    ...
                    'resource': 'aws.ami',
                    'filters': [{'type': 'value', 'key': 'DeprecationTime', 'value': 'absent'}],
                    'actions': [{'type': 'set-deprecation', 'days': -327678902}],
                }
            )
        self.assertIn("has invalid time interval", str(e.exception))
    ...
    def test_ami_with_last_launched_time(self):
        ...
        self.assertEqual(len(resources), 1)
        self.assertEqual(
            resources[0]['c7n:attribute-lastLaunchedTime']['Value'], '2022-10-20T07:07:19Z'
        )
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    {'type': 'cross-account'},
                    {'type': 'value', 'key': 'Name', 'value': 'test-ami'},
                ],
