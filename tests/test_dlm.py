...
class DLMPolicyTest(BaseTest):
    def test_dlm_query(self):
        ...
        p = self.load_policy(
            {'name': 'dlm-query', 'resource': 'dlm-policy'}, session_factory=factory
        )
        resources = p.run()
    ...
    def test_dlm_tag(self):
        ...
        p = self.load_policy(
            {
                'name': 'tag-dlm',
                'resource': 'dlm-policy',
                'filters': [{'tag:bar': 'absent'}],
                'actions': [{'type': 'tag', 'tags': {'bar': 'baz'}}],
            },
        ...
        p = self.load_policy(
            {'name': 'tag-dlm', 'resource': 'dlm-policy', 'filters': [{'tag:bar': 'present'}]},
            session_factory=factory,
        )
        resources = p.run()
        self.assertEqual(len(resources), 1)
    ...
    def test_dlm_remove_tag(self):
        ...
        p = self.load_policy(
            {'name': 'tag-dlm', 'resource': 'dlm-policy', 'filters': [{'tag:foo': 'present'}]},
            session_factory=factory,
        )
        resources = p.run()
