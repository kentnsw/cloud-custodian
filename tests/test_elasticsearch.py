...
class ElasticSearch(BaseTest):
    ...
    def test_elasticsearch_cross_cluster_search_connections(self):
        ...
        self.assertEqual(
            search_inbound_connections['CrossClusterSearchConnections'][0]['SourceDomainInfo'][
                'OwnerId'
            ],
            '644160558196',
        )
        ...
        self.assertEqual(
            search_outbound_connections['CrossClusterSearchConnections'][0]['SourceDomainInfo'][
                'OwnerId'
            ],
            '644160558196',
        )
    ...
    def test_elasticsearch_cross_cluster_search_connections_outbound(self):
        ...
        self.assertEqual(
            resources[0]['DomainName'],
            self.terraform_cross_cluster[
                'aws_elasticsearch_domain.outbound_connection.domain_name'
            ],
        )
        es = session_factory().client('es')
        search_outbound_connections = es.describe_outbound_cross_cluster_search_connections()
    ...
    def test_elasticsearch_has_statement(self):
        ...
        self.assertEqual(
            access_policy['Statement'][0]['Resource'],
        ...
        p = self.load_policy(
            {
                ...
                "actions": ["remove-matched-source-ips"],
            },
            session_factory=factory,
        )
        resources = p.run()
    ...
    def test_elasticsearch_enable_auditlog(self):
        ...
        self.assertEqual(
            state['AUDIT_LOGS']['CloudWatchLogsLogGroupArn'],
...
class TestReservedInstances(BaseTest):
    def test_elasticsearch_reserved_node_query(self):
        ...
        p = self.load_policy(
            {"name": "elasticsearch-reserved", "resource": "aws.elasticsearch-reserved"},
            session_factory=session_factory,
        )
        resources = p.run()
        self.assertEqual(len(resources), 1)
        self.assertEqual(
            resources[0]["ReservedElasticsearchInstanceId"], "036381d0-4fa5-4484-bd1a-efc1b43af0bf"
        )
