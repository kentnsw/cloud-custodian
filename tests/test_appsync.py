...
class AppSyncWafV2(BaseTest):
        ...
        p = self.load_policy(
            {
                ...
                "actions": [
                    {"type": "set-wafv2", "state": True, "web-acl": ".*FMManagedWebACLV2-?FMS-.*"}
                ],
            },
            session_factory=factory,
        )
        ...
        p = self.load_policy(
            {
                ...
                "actions": [{"type": "set-wafv2", "state": False, "force": True}],
            },
            session_factory=factory,
        )
        resources = p.run()
    ...
    def test_graphql_api_action_wafv2_regex_multiple_webacl_match(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            p.run()
        self.assertIn('matching to none or multiple webacls', str(ctx.exception))


class TestAppSyncApiCache(BaseTest):
    def test_graphql_api_cache_filter(self):
        factory = self.replay_flight_data("test_graphql_api_cache_filter")
