    ...
    p = test.load_policy(
        {
            'name': 'public-eni',
            'resource': 'aws.eni',
            'filters': [{'type': 'subnet', 'key': 'SubnetId', 'value': 'present', 'igw': True}],
        },
...
def test_cross_az_nat_gateway_subnet_resolve(test):
    ...
    subnets = {
        s['SubnetId']: s for s in p.resource_manager.get_resource_manager('aws.subnet').resources()
    }
    ...
    table_subnets = {}
    for t in tables:
        table_subnets[t['RouteTableId']] = list(cross_nat.resolve_subnets(t, subnets.values()))
...
class SecurityGroupTest(BaseTest):
    ...
    def test_unused_batch(self):
        ...
        p = self.load_policy(
            {'name': 'sg-xyz', 'resource': 'security-group', 'filters': ['unused']},
            session_factory=factory,
        )
        unused = p.resource_manager.filters[0]
        self.patch(unused, 'get_scanners', lambda: (('batch', unused.get_batch_sgs),))
        resources = p.run()
        self.assertEqual(len(resources), 1)
    ...
    def test_cidr_ingress_list(self):
        ...
        self.assertEqual(len(resources), 1)
        self.assertEqual(
            resources[0]["MatchedIpPermissions"][0]['IpRanges'][0]["CidrIp"], "192.0.0.0/32"
        )
        ...
        self.assertEqual(len(resources), 2)
        self.assertEqual(
            resources[0]["MatchedIpPermissions"][0]['IpRanges'][0]["CidrIp"], "172.0.0.0/32"
        )
        self.assertEqual(
            resources[1]["MatchedIpPermissions"][0]['IpRanges'][0]["CidrIp"], "192.0.0.0/32"
        )
