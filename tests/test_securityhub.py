...
class SecurityHubTest(BaseTest):
    ...
    def test_rds(self):
        ...
        shape_validate(
            rfinding['Details']['AwsRdsDbInstance'], 'AwsRdsDbInstanceDetails', 'securityhub'
        )

        client = factory().client("securityhub", region_name='us-west-2')
        findings = client.get_findings(
            Filters={
                'Title': [{'Value': 'rds-finding', 'Comparison': 'EQUALS'}],
                'ResourceId': [
                    {
            ...
            findings[0]["Resources"][0],
            {
                ...
                'Details': {
                    'AwsRdsDbInstance': {
                        ...
                        'VpcSecurityGroups': [
                            {'VpcSecurityGroupId': 'sg-0e255aed9188d1fba', 'Status': 'active'}
                        ],
                        ...
                        'OptionGroupMemberships': [
                            {'OptionGroupName': 'default:postgres-13', 'Status': 'in-sync'}
                        ],
                        'StorageType': 'io1',
                        'PromotionTier': 1,
                    }
