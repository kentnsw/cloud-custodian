...
class PolicyMetaLint(BaseTest):
    ...
    def test_detail_spec_format(self):
        ...
        for k, v in manager.resources.items():
            ...
            if not len(detail_spec) == 4:
                failed.append(k)
        if failed:
            self.fail("%s resources have invalid detail_specs" % ", ".join(failed))
    ...
    def test_valid_arn_type(self):
        ...
        if invalid:
            raise ValueError(
                "%d %s have invalid arn types in metadata" % (len(invalid), ", ".join(invalid))
            )
        ...
        original = policy.PolicyCollection.from_data(
            {
                "policies": [
                    ...
                    },
                    {
                        "name": "two",
                        "resource": "ec2",
                        "mode": {"type": "periodic", "role": "cutodian", "schedule": "rate(1 day)"},
                    },
                ]
            },
            cfg,
...
class ConfigModeTest(BaseTest):
    ...
    def test_config_poll_supported_resource_warning(self):
        with self.assertRaisesRegex(PolicyValidationError, r'fully supported by config'):
            self.load_policy(
                {
