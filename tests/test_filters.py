...
class TestValueFilter(unittest.TestCase):
    ...
    def test_value_type_expr(self):
        ...
        # test explicit op
        vf = filters.factory(
            {"type": "value", "value": "b", "op": 'eq', "value_type": "expr", "key": "a"}
        )
    ...
    def test_value_type_cidr(self):
        ...
        vf = filters.factory(
            {"type": "value", "value": ["abc"], "op": "in", "value_type": "cidr", "key": "ingress"}
        )
...
class TestValueTypes(BaseFilterTest):
    ...
    def test_resource_count_filter(self):
        fdata = {"type": "value", "value_type": "resource_count", "op": "lt", "value": 2}
    ...
    def test_resource_count_filter_validation(self):
        ...
        # Bad `value`
        f = {"type": "value", "value_type": "resource_count", "op": "eq", "value": "foo"}
        self.assertRaises(PolicyValidationError, filters.factory(f, {}).validate)
...
class TestReduceFilter(BaseFilterTest):
    ...
    def test_group_number(self):
        resources = self.instances()
        f = filters.factory(
            {"type": "reduce", "group-by": {"key": "Bar", "value_type": "number"}, "limit": 1}
        )
        rs = f.process(resources)
...
class ListItemFilterTest(BaseFilterTest):
    ...
    def instance(self, id_, list_):
        return {'id': id_, 'list_elements': list_}
    ...
    def test_list_item_filter(self):
        ...
        f = filters.factory(
            {'type': 'list-item', 'key': 'list_elements', 'attrs': [{'foo': 'bar'}]},
    ...
    def test_list_item_filter_match_1(self):
        ...
        f = filters.factory(
            {'type': 'list-item', 'key': 'list_elements', 'attrs': [{'foo': 'bar'}, {'bar': '1'}]},
            manager=self.get_manager(),
        )
        res = f.process(resources)
...
class AnnotationSweeperTest(unittest.TestCase):
    def test_annotation_sweep_jmespath(self):
        ...
        sweeper = AnnotationSweeper(id_key="metadata.uid", resources=resources)
    ...
    def test_annotation_sweep_jmespath_no_annotations(self):
        ...
        sweeper = AnnotationSweeper(id_key="metadata.uid", resources=swept)
